      ...
      <View style={styles.overlay}>
        <View style={[styles.card, { backgroundColor: cardBg, borderColor: border }, ultraShadow]}>
          ...
          <ScrollView style={styles.list} showsVerticalScrollIndicator={false}>
            {filtered.map((tz) => (
              <TouchableOpacity key={tz.id} activeOpacity={0.8} onPress={() => onSelect(tz.id)} style={[styles.row, { borderBottomColor: border }]}>
                <View style={{ flex: 1 }}>
                ...
                {selectedId === tz.id && <Check size={18} color={primaryColor} strokeWidth={2.3} />}
              </TouchableOpacity>
            ))}
            {filtered.length === 0 && (
...
const styles = StyleSheet.create({
  ...
  list: { maxHeight: 320 },
  row: {
    flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 12, borderBottomWidth: StyleSheet.hairlineWidth,
  },
