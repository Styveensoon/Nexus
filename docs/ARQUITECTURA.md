...
- **Aria, asistente general de onboarding/soporte continuo, que además puede actuar (con confirmación):** a diferencia de El Semillero (solo owner, solo para arrancar un proyecto nuevo), cualquier miembro puede preguntarle a Aria sobre un proyecto o tarea puntual, o en un chat general — pensado para bajar la curva de integración de gente nueva al equipo. Sobre una tarea puntual, además de responder puede proponer una acción concreta (cambiar status/fechas/prioridad, reasignar) como una card de "Aceptar/Rechazar" — nunca la ejecuta sola, y el permiso se revalida server-side antes de ofrecerla. *(Implementado: Edge Function `aria-assistant`, 3 modos de contexto validados server-side, tile propio en el Dashboard — ver `docs/ESTADO.md`.)*
...
- **Badges** discretos, gestionados por team leaders/admins con sugerencias de IA. *(Implementado: catálogo fijo de 10 badges (`BADGE_CATALOG`), otorgados/quitados por el owner de la organización o el encargado de equipo desde `BadgesScreen`/`BadgeAwardModal`, con RLS real (`profile_badges`). La sugerencia de IA (Edge Function `badge-suggestions`, Groq) analiza tasks/liderazgo/comentarios reales y propone a quién reconocer — ver `docs/ESTADO.md`.)*
...
- **Chat/comentarios** en cada task, issue, equipo y proyecto (archivos, fotos, links). *(Implementado para tasks: `task_comments` con texto, respuestas citadas, adjuntos de imagen/archivo/link/fecha, burbujas alineadas estilo WhatsApp y reacciones de 4 tipos (`task_comment_reactions`) — ver `docs/ESTADO.md`. Pendiente para issues, equipos y proyectos.)*
